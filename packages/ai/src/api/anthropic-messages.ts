...
export const streamSimple: StreamFunction<"anthropic-messages", SimpleStreamOptions> = (
	...
	if (!options?.reasoning) {
		return stream(model, context, { ...base, toolChoice, thinkingEnabled: false } satisfies AnthropicOptions);
	}
...
function convertTools(
	...
	return tools.map((tool, index) => {
		// Server tools (e.g. tool_search_tool_20250919) need type passthrough and no
		// input_schema or description: the API rejects extra fields on server tools.
		if ((tool as any).serverTool && (tool as any).type) {
			return {
